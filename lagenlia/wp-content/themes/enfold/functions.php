...
if (!function_exists('avia_register_frontend_scripts')) {
    ...
    function avia_register_frontend_scripts() {
        ...
        wp_enqueue_script('my_customjs', $template_url . '/js/custom.js', array('jquery', 'avia-default'), $vn, true);
        ...
        wp_register_style('my-custom', $template_url . "/", array(), $vn, 'all');
       ...
       // wp_enqueue_style('owl.carousel', $template_url . "/css/owl.carousel.min.css", array('avia-layout'), $vn, 'all');
...
//My include
include get_template_directory() . '/event/event_shortcode.php';
include get_template_directory() . '/event/upcp_loop.php';



// Singlepage acton
function event_content_single_page() {
    if (is_single() && !empty($GLOBALS['post'])) {
        if ($GLOBALS['post']->ID == get_the_ID()) {
            $std = get_field('event_start_date');
            $date = DateTime::createFromFormat('Ymd', $std);
           // $date = DateTime::createFromFormat('Ymd', $std);
            if (!empty($std)):
                ?>
                <div class="wrap">
                    <h5 class="av-share-link-description av-no-toc ">Event Info</h5>
                    <div class="time"><span><strong>Event Start At: </strong> <?php echo $date->format('j M Y');  ?> <?php echo the_field('event_start_time'); ?> </span><span> <strong>Event End At: </strong> <?php echo the_field('event_end_date'); ?> </span>
                    </div>
                    <div class="loc"><strong>Venue: </strong> <?php echo the_field('event_location'); ?></div>
                </div>
...
add_action('ava_after_content', 'event_content_single_page');
