	...
	function avia_events_dynamic_css( $output, $color_set )
	{
		...
		foreach( $color_set as $key => $colors ) // iterates over the color sets: usually $key is either: header_color, main_color, footer_color, socket_color
		{
			...
			if( $key == '.main_color' )
			{
				...
				.single-tribe_events $key  .tribe-events-cost, $key #tribe-events-content .tribe-events-calendar td, $key .tribe-events-adv-list-widget .tribe-events-event-cost, #top #wrap_all $key .tribe-events-event-meta a{ 
					color: $primary; 
				}
